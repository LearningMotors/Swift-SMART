					...
					if nil != scanner.fhir_scanString("Z") {
						...
						if let hourStr = scanner.fhir_scanCharacters(from: digitSet) {
							...
							if 2 == hourStr.count {
								...
								tzhour = Int(hourStr.prefix(upTo: hourStr.index(hourStr.startIndex, offsetBy: 2)))!
								tzmin = Int(hourStr.suffix(from: hourStr.index(hourStr.startIndex, offsetBy: 2)))!
							}
